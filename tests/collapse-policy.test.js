...
import {
  createCollapsePolicy,
  COLLAPSE_MODES,
} from '../src/catalan/collapse-policy.js';
...
test(
  'heavier policy keeps the larger subtree and occasionally the lighter one',
  () => {
...
test('left/right policies deterministically choose a side', () => {
  const leftPolicy = createCollapsePolicy(countPairs, {
    mode: COLLAPSE_MODES.LEFT,
  });
  const rightPolicy = createCollapsePolicy(countPairs, {
...
test(
  'freezeBalanced keeps nodes whose children are within the threshold',
  () => {
