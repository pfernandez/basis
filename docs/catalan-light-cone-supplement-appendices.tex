...
same outcome.
If $f$ is prefix-local in the sense of Remark~\ref{CLC-rem:local-observables}, then
...
Section~\ref{CLC-sec:multiplecones}), which may be studied by counting completions
as a function of current height (Lemma~\ref{CLC-lem:ballot-completions}).
\end{remark}
...
\]
	...
	Lemma~\ref{CLC-lem:ballot-completions}. The associated future-cone entropy is
\[
...
equivalence relation $\sim_g$ of Definition~\ref{CLC-def:gauge-equivalence}. When
...
% Optional operator/field frameworks previously here were moved to
% supplemental-operators.tex (not included in the arXiv v1 PDF).
...
\begin{proof}[Proof Sketch (Catalan universality for program structure)]
